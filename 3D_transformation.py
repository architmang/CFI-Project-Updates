...
# between azure_kinect1_2 color to azure_kinect1_4 depth
# between azure_kinect1_3 color to azure_kinect1_4 depth
# between azure_kinect1_4 color to azure_kinect1_4 depth
...
pairwise_color2depth_extr_param = pickle.load(open(new_color2depth_extr_pickle_file, 'rb'))
...
for cam in cam_list:
    ...
    mtx_d = np.array([[intr_param['%s_calib_snap_depth' % cam4][0], 0, intr_param['%s_calib_snap_depth' % cam4][2]],
                      [0, intr_param['%s_calib_snap_depth' % cam4][1], intr_param['%s_calib_snap_depth' % cam4][3]],
                      [0, 0, 1]])
    dist_d = intr_param['%s_calib_snap_depth' % cam4][6:14]
    # print(mtx_d, '\n', dist_d)
    ...
    R, T = pairwise_color2depth_extr_param["%s_color2_%s_depth" % (cam, cam4)]
    M = np.eye(4)
    M[0:3, 0:3] = R
